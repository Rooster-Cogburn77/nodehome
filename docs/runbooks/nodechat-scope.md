...
1. **Hallucination eval regression loop.** Live homelab baseline smoke is captured. Re-run the corpus after any model/profile/routing change, and add formal manual-score recording or a structured claim-against-evidence scorer before treating the eval as an automated quality metric.
