...
- **Auto-routing for AI History and repo files**, conservative Observe-tier heuristics. History routes on prior-decision phrasing (`what did we …`, `remind me`, `previously`, `history of …`, `prior decision/incident/run`, etc.); repo routes on concrete artifacts (named files like `CURRENT_STATE`/`SESSION_LOG`/`CLAUDE.md`/`SCRATCH.md`/`ATTITUDE.md`, known runbook stems, and path-like tokens such as `docs/...`/`scripts/...`/`sweeps/...`/`tests/...`) plus high-confidence codebase/project summary prompts that combine a repo subject with summary/progress/stack/completed/outstanding intent. Summary prompts load `docs/CURRENT_STATE.md` and `docs/wiki/concepts/full-stack-inventory.md`. Vague topic phrases and bare filenames do not auto-route; users invoke `/read` for those. Repo auto-routing reads at most two files per turn, applies the same workspace confinement and secret-path refusals as `/read`, and never blocks the chat turn on routing failure.
...
- **Hallucination-friction controls.** `tests/nodechat_eval_corpus.py` and `scripts/nodechat_eval.py` define a reproducible eval corpus and local JSONL eval output under gitignored `runtime/nodechat/evals/`. The corpus pins high-severity vs project-fact unsupported-claim taxonomy, with zero tolerance for invented files/functions/commands/commits/runbooks/config values and a `<5%` project-fact unsupported target. Runtime controls include route-aware generation params from existing route signals, per-turn `NODECHAT_EVIDENCE_STATE` with source labels, a downstream answerability gate for zero-evidence project-specific prompts, named-artifact/commit relevance gating when unrelated evidence is loaded, force overrides via `answer anyway:` and `--force-answer`, and detection-only unsupported-claim review logging. The eval harness can now generate a manual `*.scores.jsonl` overlay and summarize it against the pinned thresholds, keeping correctness scoring human-grounded instead of model-judged. Homelab live eval `nodechat-eval-20260520-011100.jsonl` completed with `gated: 7` / `ok: 11` after the eval harness and relevance-gate fixes. This is friction and measurement, not a guarantee of hallucination prevention.
- Safety test suite in `tests/test_nodechat_safety.py` plus eval tests are passing; full stdlib discovery is `131/131` as of the 2026-05-19 homelab eval pass.
- **Auto-routing recall pass — Phase A (measurement infrastructure) shipped.** Labeled corpus of 102 realistic prompts in `tests/routing_corpus.py` covering history positives (incl. Phase B widening targets), repo positives (named files / runbook stems / path tokens / multi-file / summary-intent docs), web positives (URL fetch + fresh-public search), live positives (one per check), neutral/general prompts, and adversarial guardrails. Harness emits per-router precision/recall + FP/FN lists and runs as both a CLI (`py -3 tests/routing_corpus.py`) and a regression test class.
...
- **Broader operator approvals shipped.** `/live` now exposes a small allowlisted Mutate-tier surface alongside the existing Observe-tier reads. Diagnostics (`/live ps`, `/live logs <vllm|open-webui|ollama>`, `/live journal ollama`, `/live inspect <vllm|open-webui>`) run immediately. Mutations (`/live restart vllm-server`, `/live restart open-webui`, `/live restart ollama`) queue an approval row of class `live-mutation` and only execute after `/approve <id>`. Ollama restart uses the exact validated argv `sudo -n /bin/systemctl restart ollama`. Hard guardrails: no arbitrary container names, no arbitrary journalctl units, no templated systemd units, no `--follow`, no shell composition, no restart without `/approve`, and no Windows-local queueing of Linux-only mutation argv. New audit events: `live_diag_executed`, `live_mutation_refused`, `live_mutation_queued`, `live_mutation_executed`, `live_mutation_blocked`. Full reference: [`live-mutations.md`](live-mutations.md).
...
- Additional Mutate-tier ops beyond the current restart allowlist. Specifically: `docker compose up/down` (waits until a compose-managed service exists in this repo/host layout), config edits to `/etc/systemd/system/*.service.d/override.conf`, BMC password rotation, etc. Each new op needs a fixed argv, a runbook entry, and a regression test.
...
1. **Hallucination eval regression loop.** Live homelab baseline smoke is captured. Re-run the corpus after any model/profile/routing change, fill the manual score overlay for `ok` rows, and use the review summary before treating a change as a quality improvement. A structured claim-against-evidence scorer remains a future helper, not the source of truth.
2. **Auto-routing recall pass — corpus growth.** Phase B is complete (all routers 1.00 / 1.00 on the 102-prompt corpus). The next iteration is corpus growth: add prompts surfaced by real Nodechat use that don't currently route the way the user expects. This is now a maintenance loop, not a discrete lane.
3. **Broader operator approvals — additional ops.** Current restart allowlist covers `vllm-server`, `open-webui`, and `ollama`, with local-target validation before queueing. Narrow `docker compose up -d <service>` should wait until a compose-managed service exists in this repo/host layout. Each new op follows the same pattern: fixed argv in `LIVE_MUTATION_OPS`, runbook entry, regression test that asserts queue + execute + audit.
4. **Model routing refinement.** Remote profiles are in place. Next refinements are provider usage-accounting when available, live smoke validation with real keys only when intentionally enabled, and explicit local/remote routing policy improvements after real use.
