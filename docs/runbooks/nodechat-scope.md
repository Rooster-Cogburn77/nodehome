...
- Phase A + Phase B (history + web) baseline: repo 1.00/1.00; history 1.00/1.00; web 1.00/1.00; live 0.78 / 0.68. Two guardrails remain in `PHASE_B_GUARDRAIL_TARGETS` (live g006 + g007, scheduled for the next Phase B commit).
...
- Auto-routing recall pass — Phase B live (heuristic widening based on Phase A's FP/FN rows). Phase B targets: precision ≥ 0.95 and recall ≥ 0.95 on every router; zero guardrail failures.
...
1. **Auto-routing recall pass — Phase B live (heuristic widening).** History done (1.00 / 1.00). Web done (1.00 / 1.00). Live next: add "box" to `LIVE_OBJECT_RE` (g007), narrow the over-broad `health` fallback that fires alongside specific checks (l001/l002/l007/g006), and prevent live from firing when web-explicit signals like "online" / "look up" / "verify online" are present (w102/w107/w110). Re-measure; ratchet floors upward toward 0.95 / 0.95.
