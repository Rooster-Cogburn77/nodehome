...
**Status:** IN PROGRESS — Phase 1 password rotation complete; SSDP disabled; BMC SEL clock manually corrected while powered; cert hygiene, VBAT replacement, and network phases pending.
...
6. **[blocked / inspected 2026-05-18]** Replace the BMC HTTPS certificate with a certificate/private-key pair matching the final BMC hostname or static IP. The web UI is reachable today through the USB-NIC tunnel (`https://169.254.3.254/`), and `Configuration -> Network -> SSL Certificates` exposes upload fields for `New SSL Certificate` and `New Private Key` only; no CSR or self-signed generator was visible. Do not upload yet: the final hostname/static IP is undecided and `VBAT` is failed. The BMC SEL clock was manually corrected on 2026-05-18 with `sudo ipmitool sel time set now`, but persistence across power loss is suspect until `VBAT` is replaced. Use unencrypted PEM, RSA 2048, when ready.
...
- `Configuration -> BMC Settings -> Date and Time`: NTP is off, timezone is UTC, NTP servers are `localhost` and `127.0.0.1`, and the UI initially displayed stale time (`2025-04-01T00:46:16Z`; later `sudo ipmitool sel time get` showed `04/01/2025 01:44:20 AM UTC`). Host time was synchronized (`2026-05-18T03:06:39Z`, NTP active), but no UDP `:123` listener was present on the host USB-NIC path, so BMC NTP was not enabled. `sudo ipmitool sel time set "$(date -u '+%m/%d/%Y %H:%M:%S')"` failed with `Specified time could not be parsed`; `sudo ipmitool sel time set now` succeeded and `sudo ipmitool sel time get` returned `05/18/2026 03:06:39 AM UTC`. Treat BMC timestamps as sane while powered, but persistence across power loss is suspect until `VBAT` is replaced.
