...
**Status:** IN PROGRESS — Phase 1 password rotation complete; SSDP disabled; VBAT battery replaced and sensor clear; BMC SEL clock manually corrected while powered; cert hygiene and network phases pending.
...
6. **[blocked / inspected 2026-05-18]** Replace the BMC HTTPS certificate with a certificate/private-key pair matching the final BMC hostname or static IP. The web UI is reachable today through the USB-NIC tunnel (`https://169.254.3.254/`), and `Configuration -> Network -> SSL Certificates` exposes upload fields for `New SSL Certificate` and `New Private Key` only; no CSR or self-signed generator was visible. Do not upload yet: the final hostname/static IP is undecided. The BMC SEL clock was manually corrected on 2026-05-18 with `sudo ipmitool sel time set now`; VBAT is now clear after CR2032 install, but clock retention should still be verified on the next full power-loss check. Use unencrypted PEM, RSA 2048, when ready.
...
- `Configuration -> BMC Settings -> Date and Time`: NTP is off, timezone is UTC, NTP servers are `localhost` and `127.0.0.1`, and the UI initially displayed stale time (`2025-04-01T00:46:16Z`; later `sudo ipmitool sel time get` showed `04/01/2025 01:44:20 AM UTC`). Host time was synchronized (`2026-05-18T03:06:39Z`, NTP active), but no UDP `:123` listener was present on the host USB-NIC path, so BMC NTP was not enabled. `sudo ipmitool sel time set "$(date -u '+%m/%d/%Y %H:%M:%S')"` failed with `Specified time could not be parsed`; `sudo ipmitool sel time set now` succeeded and `sudo ipmitool sel time get` returned `05/18/2026 03:06:39 AM UTC`. After CR2032 install and power cycle, BMC time came back stale (`03/28/2025 12:41:09 PM UTC`), so `sudo ipmitool sel time set now` was run again at `host_utc=2026-05-18T03:40:28Z`; `set_exit=0` and BMC returned `05/18/2026 03:40:28 AM UTC`. Immediate later spot-check returned BMC `05/18/2026 03:44:02 AM UTC` and host `2026-05-18T03:44:03Z`, proving the BMC clock is tracking host UTC within one second while powered. Verify retention on the next full power-loss check.
