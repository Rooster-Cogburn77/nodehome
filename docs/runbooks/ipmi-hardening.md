...
**Status:** IN PROGRESS — Phase 1 password rotation complete; SSDP disabled; VBAT battery replaced and sensor clear; BMC SEL clock manually corrected while powered; BMC NTP configured over the host USB-NIC link; cert hygiene and network phases pending.
...
4. **Fallback posture if owned routing/VLANs cannot land yet.** Temporary double NAT is acceptable during a router transition, but it is not the final BMC management posture. Keep the BMC on the in-band USB-NIC path with Phase 1 hardening as the only security boundary and skip Phases 2 and 3 until owned gateway/switch/AP gear is installed.
...
6. **[blocked / inspected 2026-05-18; time base cleaned up 2026-05-19]** Replace the BMC HTTPS certificate with a certificate/private-key pair matching the final BMC hostname or static IP. The web UI is reachable today through the USB-NIC tunnel (`https://169.254.3.254/`), and `Configuration -> Network -> SSL Certificates` exposes upload fields for `New SSL Certificate` and `New Private Key` only; no CSR or self-signed generator was visible. Do not upload yet: the final hostname/static IP is undecided. The BMC SEL clock was manually corrected on 2026-05-18 with `sudo ipmitool sel time set now`; on 2026-05-19 the host was configured to serve Chrony only on `169.254.3.1:123`, the BMC Date/Time UI was set to NTP server `169.254.3.1` with timezone UTC, and SEL time verified against host UTC within a few seconds. VBAT is clear after CR2032 install, but clock retention should still be verified on the next full power-loss check. Use unencrypted PEM, RSA 2048, when ready.
...
**Cost:** $0 for steps 1-5 and SSDP/clock work; CR2032 was already on hand. **Time:** steps 1-5 actual = ~30 min (2026-05-17); cert upload adds ~15 min after hostname/static-IP is settled. **Outcome of steps 1-5 (completed 2026-05-17):** factory password is no longer the live credential; live credential lives only in the KeePassXC vault entry `Nodehome - Supermicro H12SSL-i BMC`; rotation verified by authenticated RMCP+ login (`-I lanplus -C 3`) against `169.254.3.254`. Cert hygiene remains pending.
...
- `Configuration -> BMC Settings -> Date and Time`: initial inspection showed NTP off, timezone UTC, NTP servers `localhost` and `127.0.0.1`, and stale BMC time (`2025-04-01T00:46:16Z`; later `sudo ipmitool sel time get` showed `04/01/2025 01:44:20 AM UTC`). Host time was synchronized (`2026-05-18T03:06:39Z`, NTP active), but no UDP `:123` listener was present on the host USB-NIC path, so BMC NTP was not enabled then. `sudo ipmitool sel time set "$(date -u '+%m/%d/%Y %H:%M:%S')"` failed with `Specified time could not be parsed`; `sudo ipmitool sel time set now` succeeded and `sudo ipmitool sel time get` returned `05/18/2026 03:06:39 AM UTC`. After CR2032 install and power cycle, BMC time came back stale (`03/28/2025 12:41:09 PM UTC`), so `sudo ipmitool sel time set now` was run again at `host_utc=2026-05-18T03:40:28Z`; `set_exit=0` and BMC returned `05/18/2026 03:40:28 AM UTC`. Immediate later spot-check returned BMC `05/18/2026 03:44:02 AM UTC` and host `2026-05-18T03:44:03Z`, proving the BMC clock tracked host UTC within one second while powered. On 2026-05-19, Chrony was configured on the host with `/etc/chrony/conf.d/nodehome-bmc-ntp.conf` (`bindaddress 169.254.3.1`, `allow 169.254.3.0/24`); `ss` verified `169.254.3.1:123`; `chronyc tracking` reached `Leap status: Normal`; the BMC UI was set to NTP server `169.254.3.1` and timezone UTC. Verification: `host_utc=2026-05-19T17:24:08Z`; `sudo ipmitool sel time get` returned `05/19/2026 05:24:12 PM UTC`. NTP is configured; verify retention on the next full power-loss check.
...
The home gateway was checked before buying network gear. Local default gateway `192.168.1.1` is a Spectrum/Sagemcom `SAX1V1S` Advanced WiFi router. The router info page exposed model/firmware details; the public WAN IP and serial were intentionally not recorded in repo. User physically confirmed a separate Spectrum modem feeds the router over ethernet.
...
1. Buy/install the owned gateway/switch/AP stack.
2. Connect the owned gateway WAN directly to the Spectrum modem and power-cycle the modem so it learns the new WAN MAC.
3. Remove/return the Spectrum router if it is no longer needed for service.

Do not patch the BMC dedicated NIC into the flat LAN before the owned management network exists.
...
2. **[blocked / inspected 2026-05-18]** BMC web UI presents a cert that at least correctly identifies the BMC's hostname/IP. The web UI is reachable today over the USB-NIC tunnel, but final cert upload is gated on the target hostname/static IP. BMC clock is manually corrected and VBAT is clear; verify clock retention on the next full power-loss check.
