...
**Current state:** Two 12TB drives are in the landing/validation phase after Walmart canceled the original Drive #1. Replacement Drive #1 is a low-hour WD Easystore 12TB from sv2deals (`54` hours in seller SMART screenshot, serial `5PJHV96C`) and still needs arrival verification. Drive #2, the PayMore WD My Book 12TB, arrived 2026-05-18 and is accepted: `/dev/sda` / `WDC WD120EDGZ-11CMZA0` / serial `T3G0WU1E`; SMART identifies it as Western Digital Ultrastar (He10/12) family, 12.0 TB, 7200 rpm helium, with clean final counters (`19` POH, `6` power cycles, zero reallocated/pending/uncorrectable/CRC), passed short and extended self-tests, and saved final artifact `~/drive-check-mybook-T3G0WU1E-final.txt`. It is ext4-formatted as `mybook12tb`, mounted at `/mnt/media/mybook12tb`, and fstab-verified by UUID `75589f25-4dad-42da-b0ef-31187e14eaa3`. If both drives validate, the node has 24 TB total bulk capacity plus 2 TB NVMe, comfortably matching near-term household media + photo + backup workload.
