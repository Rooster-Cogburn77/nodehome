...
1. **Staged 2-GPU vLLM thermal soak** — 2026-05-14 status: complete/pass for watched 30-minute stock-power validation and top-fan A/B. No-top-fan run completed 180 iterations with GPU0 topping at `84 C` / `90% fan`; top-fan repeat completed 180 iterations with GPU0 at `77 C` / `82% fan`, GPU1 at `71 C` / `72% fan`, GPU2 isolated at `1 MiB` / `0% util`, and post-run healthcheck `[HEALTHY] no failures, no warnings`. Top fan stays on for sustained 2-GPU vLLM work. Next useful validation: capture 350W wall-clock request baseline, then test `300 W` power cap with the same prompt before deciding whether to make a persistent systemd power-cap unit.
