...
## Current status
- **Commits on `origin/main`:** `938f3cd` (scope), `e0c83da` (auto-routing), `2a71947` (undo), `9ef17b0` (log refresh), `a13d374` (web), `4dfb050` (live-node operator), `6168e78` (grouped evidence + hard rule), `ff05e8f` (recall pass Phase A), `898093c` (Phase B history). Phase B web is in working tree, not yet committed.
- **Nodechat capability lanes done:** scope correction, AI History + repo auto-routing with disclosure / context controls / structured provenance / audit, `/undo-apply` with freshness check + safety backup + Windows CRLF fix, web auto-routing + `/web-mode`, live-node Observe checks + `/live-mode`, grouped `/evidence`, routing recall Phase A (measurement infrastructure), Phase B history (1.00 / 1.00), Phase B web (1.00 / 1.00, working tree).
...
1. Auto-routing recall pass — Phase B live. Add "box" to `LIVE_OBJECT_RE` (g007), narrow over-broad `health` fallback that fires alongside specific checks (l001/l002/l007/g006), prevent live firing when web-explicit signals like "online"/"look up"/"verify online" are present (w102/w107/w110). Re-measure; ratchet floors upward toward 0.95 / 0.95.
...
- **Network replacement follow-up:** before buying, confirm whether the ISP device can be placed into bridge/modem mode. If yes, target UniFi Cloud Gateway Ultra + UniFi Switch Lite 8 PoE + U7 Lite AP (~$337 before tax/cables). If no, decide whether to replace the cable modem separately or run double NAT temporarily.
