...
- **Tests:** 45/45 passing (13 prior + 12 first auto-routing + 2 reliability regressions + 4 undo/apply-backup + 4 web-routing + 5 live-operator + 1 grouped-evidence + 4 routing-corpus). Includes the user-mandated "vague repo topic does not route" guardrail, a local-status web false-positive guardrail, a live-status-vs-history false-positive guardrail, and the new corpus-driven regression ratchets.

## Current status
- **Commits on `origin/main`:** `938f3cd` (scope), `e0c83da` (auto-routing), `2a71947` (undo), `9ef17b0` (log refresh), `a13d374` (web), `4dfb050` (live-node operator), `6168e78` (grouped evidence + hard rule). Recall pass Phase A is in working tree, not yet committed.
- **Nodechat capability lanes done:** scope correction, AI History + repo auto-routing with disclosure / context controls / structured provenance / audit, `/undo-apply` with freshness check + safety backup + Windows CRLF fix, web auto-routing + `/web-mode`, live-node Observe checks + `/live-mode`, grouped `/evidence`, routing recall Phase A (working tree).
...
1. Auto-routing recall pass — Phase B (heuristic widening, router-by-router). Read the FP/FN lists from `py -3 tests/routing_corpus.py` and the six entries in `PHASE_B_GUARDRAIL_TARGETS`. Propose targeted pattern additions/tightening per router. Re-measure after each router; ratchet floors upward toward 0.95 / 0.95.
