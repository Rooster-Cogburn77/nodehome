...
1. **Staged 2-GPU vLLM thermal soak** — 2026-05-14 status: complete/pass for watched 30-minute stock-power validation. API request path passed, 10-request watched loop passed, full 180-iteration `/tmp/vllm-soak.sh` run completed, GPU 2 stayed isolated at `1 MiB` / `0% util`, and post-soak `./scripts/healthcheck.sh` returned `[HEALTHY] no failures, no warnings`. Caveat: stock `350 W` profile reached GPU0 `84 C` / `90% fan`, so next useful validation is a `300 W` power-cap A/B before any overnight/routine production soak.
