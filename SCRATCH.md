...
- **Auto-routing recall pass — Phase B history landed.** Patterns split into TIGHT (project-bound by phrasing) and BROAD (require co-occurrence with new `HISTORY_PROJECT_CONTEXT_RE`). Three new broad patterns added: `have we ever`, `has X ever`, `what was our reasoning`. History router 0.81 / 0.81 → **1.00 / 1.00**. Three history guardrails removed from `PHASE_B_GUARDRAIL_TARGETS`; three remain (web g008 + live g006/g007).
- **Tests:** 45/45 passing.

## Current status
- **Commits on `origin/main`:** `938f3cd` (scope), `e0c83da` (auto-routing), `2a71947` (undo), `9ef17b0` (log refresh), `a13d374` (web), `4dfb050` (live-node operator), `6168e78` (grouped evidence + hard rule), `ff05e8f` (recall pass Phase A). Phase B history is in working tree, not yet committed.
- **Nodechat capability lanes done:** scope correction, AI History + repo auto-routing with disclosure / context controls / structured provenance / audit, `/undo-apply` with freshness check + safety backup + Windows CRLF fix, web auto-routing + `/web-mode`, live-node Observe checks + `/live-mode`, grouped `/evidence`, routing recall Phase A (measurement infrastructure), routing recall Phase B history (1.00 / 1.00, working tree).
...
1. Auto-routing recall pass — Phase B web. Tighten `WEB_LOCAL_ONLY_RE` so prompts like "is open webui currently running in a container", "current nvidia gpu power draw across all three cards", and "the latest model we trained" don't trip the search heuristic. Then Phase B live: add "box" to `LIVE_OBJECT_RE`, narrow over-broad health fallback, prevent live firing when web-explicit "online"/"look up" signals are present. Re-measure per router; ratchet floors upward toward 0.95 / 0.95.
